...
class CVRiderProcessor extends AudioWorkletProcessor {
  ...
  updateCoefficients(P) {
    ...
    if (changed('output'))       this.outputLin = dbToLin(P.output[0]);
  }
  ...
  process(inputs, outputs, P) {
    ...
    const idleThr = P.idleThreshold[0], monitor = Math.round(P.monitor[0]), bypass = P.bypass[0] > 0.5;
    ...
    for (let i = 0; i < n; i++) {
      ...
      const h = this.hp.process(x), h2 = h * h, x2 = x * x;
      ...
      gainDb = c * (gc + consTrim) + (1 - c) * (gv + vowelTrim);
      let gain = dbToLin(gainDb) * this.outputLin;
      if (monitor === 1) gain = c; else if (monitor === 2) gain = 1 - c;
      ...
      for (let ch = 0; ch < numCh; ch++) {
        ...
        const delayed = d[readPos];
        output[ch][i] = bypass ? delayed : delayed * gain;
      }
    ...
    if (this.meterCounter >= this.fs * 0.02) {
      ...
      this.meters.vowelGainDb = gv + vowelTrim; this.meters.consGainDb = gc + consTrim;
      this.meters.gainDb = bypass ? 0 : gainDb + P.output[0];
